    ...
    <div className="min-h-screen bg-[#080808] text-white/90 font-serif pb-64">
      ...
      <main className="flex flex-col">
        {Object.entries(groupedMasters).map(([date, cards], index) => (
          <section 
            key={date} 
            className="sticky bg-[#080808] border-t border-white/10 pt-10 pb-20 shadow-[0_-20px_40px_rgba(0,0,0,0.8)]"
            style={{ 
              top: `${80 + index * 4}px`, // スクロール時に少しずつずれて重なる
              zIndex: 100 - index 
            }}
          >
            {/* 日付ラベル */}
            <div className="px-10 mb-8 flex items-center gap-6">
              <div className="flex flex-col">
                ...
                <span className="text-[10px] font-mono opacity-30 uppercase">{date.replace(/\//g, ' . ')}</span>
              </div>
            ...
            <div className="flex overflow-x-auto gap-10 px-10 no-scrollbar pb-10 mask-fade-right">
              {cards.map((m) => (
                <div key={m.id} className="flex-shrink-0 transition-all duration-500 hover:scale-105 hover:-translate-y-2">
      ...
      <footer className="fixed bottom-10 left-0 right-0 flex justify-center z-[1001]">
         <div className="bg-[#E5E5DE] text-black px-8 py-3 rounded-sm border-2 border-black font-black uppercase italic text-[10px] tracking-[0.4em] shadow-2xl cursor-pointer hover:bg-white active:scale-95 transition-all">
            Enter Junk Yard
         </div>
      </footer>
    </div>
...
function CardSurface({ m, myId }: { m: any, myId: string }) {
  ...
  return (
    <div className="w-[220px] aspect-[1/1.618] bg-[#FAFAFA] rounded-[10px] p-[7px] shadow-[0_20px_40px_rgba(0,0,0,0.6)] flex flex-col relative overflow-hidden group">
      <div className="absolute inset-[7px] rounded-[4px] bg-[#111]" />
      <div className="relative z-10 flex flex-col h-full gap-1">
        {/* カード内ヘッダー */}
        <div className="flex gap-1 h-9">
          <div className="bg-[#E5E5DE] text-black w-9 flex items-center justify-center font-black text-[10px] border border-black italic">
            {myInstance ? `#${String(myInstance.serial_number).padStart(3, '0')}` : '??'}
          </div>
          <div className="bg-[#E5E5DE] text-black flex-1 flex items-center px-2 border border-black font-black text-[9px] italic tracking-tighter uppercase">
            Rubbish
          </div>
        </div>
        ...
        <div className="bg-black p-1 border border-black">
          <img src={m.image_url} className="w-full aspect-[1.2/1] object-cover saturate-100 contrast-110 brightness-105 transition-transform duration-700 group-hover:scale-110" />
        </div>
        ...
        <div className="flex-1 border border-black p-3 relative overflow-hidden" style={{ background: `linear-gradient(165deg, ${color} 0%, #000 100%)` }}>
          ...
          <div className="relative z-10 flex flex-col h-full justify-between uppercase font-mono text-[7px] tracking-tighter">
            <div className="space-y-1 opacity-50">
              <p className="border-b border-white/10 pb-1 truncate">ID: {m.creator_id}</p>
              <p>STAMP: {new Date(m.created_at).toLocaleDateString()}</p>
            </div>
            <div className="text-[8px] italic font-black text-right opacity-20">
               {myInstance ? "COLLECTED" : "WILD_JUNK"}
            </div>
          </div>
        </div>
      </div>
