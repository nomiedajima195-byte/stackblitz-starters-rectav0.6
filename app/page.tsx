...
export default function Room134_90s() {
  ...
  return (
    <div className="min-h-screen text-[#000] font-mono overflow-x-hidden selection:bg-[#000080] selection:text-white relative">
      ...
      {/* Header: Dot感あるRoom134 & ◎upload. 位置固定。テキスト色Room134はオレンジ、◎uploadは白 */}
      ...
      {/* Footer: Win98 Like Navigation. 背景グレー、◎Roomボタンテキスト黒。四角いWin98風ボタン。◎ボタンないテキスト色は黒 */}
      {!viewingNode && (
      ...
      {/* Viewing Node Interface: 大きなグレーの四角（Win98ウィンドウ）の中で再生。⇄ Remix, ↓ PADボタンも Win98風。 */}
      {viewingNode && (
        <div className="fixed inset-0 z-[5000] bg-[#808080]/80 flex items-center justify-center p-4 animate-in fade-in duration-500">
          <div className="win-btn bg-[#c0c0c0] w-full max-w-4xl max-h-[90vh] flex flex-col shadow-2xl">
            ...
            <div className="p-6 overflow-auto bg-white flex-grow flex flex-col items-center justify-center min-h-[300px]">
              {viewingNode.image_url === 'TRACK_TYPE' ? (
                <div className="w-full h-full flex flex-col items-center gap-4">
                  <div className="flex-grow w-full flex items-center justify-center bg-black/5 win-btn p-1 shadow-sm">
                ...
                <div className="w-full h-full flex flex-col items-center gap-4">
                    {viewingNode.image_url !== 'NODE' ? (
                        ...
                        <div className="p-10 win-btn p-1 bg-white break-words text-xl italic text-black/80">{viewingNode.description}</div>
                    )}
  ...
  return (
    <div className="flex flex-col items-center w-full h-full justify-center">
      {current?.image_url && !['NODE'].includes(current.image_url) ? (
        ...
        <div className="p-20 text-xl italic text-center text-black/80 break-words animate-in fade-in duration-300">
          {current?.description}
        </div>
      )}
    </div>
  ...
  return (
    <div className="w-full h-full flex items-center overflow-x-auto p-4 no-scrollbar space-x-4">
      {data.map((item: any, i: number) => (
        <div key={i} className="flex-shrink-0 win-btn p-1 bg-white border-inset">
            <img src={item.image_url} className="h-[60vh] object-contain" />
        </div>
      ))}
      <label className="flex-shrink-0 h-[60vh] aspect-[3/4] win-btn bg-[#d0d0d0] flex items-center justify-center text-5xl opacity-30 cursor-pointer hover:opacity-100 hover:bg-[#dfdfdf] transition-all">
        ＋
  ...
  return (
    <div className="fixed inset-0 z-[4500] bg-[#808080]/80 flex items-center justify-center p-4 animate-in fade-in">
      <div className="win-btn p-4 w-full max-w-xl">
        ...
        {file && <p className="text-[8px] text-gray-500 mt-1 italic break-words">>> {file.name}</p>}
      </div>
    </div>
  );
}
...
function BoxCreator({onRelease, onCancel}: any) {
  ...
  return (
    <div className="fixed inset-0 z-[4500] bg-[#808080]/80 flex items-center justify-center p-4 animate-in fade-in">
      <div className="win-btn p-4 w-full max-w-4xl shadow-2xl">
        <div className="bg-[#000080] text-white p-1 px-2 mb-4 flex justify-between items-center text-[11px] font-bold">
        ...
        <div className="grid grid-cols-6 gap-2 mb-6 h-60 overflow-y-auto p-2 bg-[#808080] border-inset">
          {boxItems.map((item, i) => (
            <div key={i} className="aspect-[3/4] win-btn bg-white overflow-hidden flex items-center justify-center border-inset relative">
              {item ? <img src={item.image_url} className="w-full h-full object-cover" /> : <label className="w-full h-full flex items-center justify-center text-3xl opacity-10 cursor-pointer hover:opacity-40 transition-opacity">＋<input type="file" className="hidden" onChange={e => e.target.files?.[0] && handleFile(i, e.target.files[0])} /></label>}
            </div>
          ))}
        </div>
