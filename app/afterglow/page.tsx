...
export default function EngineShareAdded() {
      ...
      <main className="flex-grow relative flex flex-col overflow-hidden bg-[#dcdcdc] p-2">
        {(mode === 'MAIN' || mode === 'WIKI' || mode === 'KEEP') && (
          <div className="flex flex-col h-full relative">
            ...
            >
              {/* CURRENT ITEM */}
              {((mode === 'MAIN' ? streetPost : (mode === 'KEEP' ? keeps[currentIndex % (keeps.length || 1)] : wikiData))) ? (
                <article>
                  <div className="flex justify-between items-center mb-6">
                    <span className="text-[10px] font-black uppercase italic opacity-30">
                      {mode === 'MAIN' ? 'STREET' : mode === 'KEEP' ? 'ALLEY' : 'WIKI'}
                    </span>
                    {/* KEEPモードの時だけ出るSHAREボタン */}
                    {mode === 'KEEP' && (
                      <button 
                        onClick={() => handleShare(keeps[currentIndex % keeps.length])}
                        className="text-[9px] font-black border border-black px-2 py-0.5 active:bg-black active:text-white transition-colors"
                      >
                        SHARE
                      </button>
                    )}
                ...
                <div className="h-full flex items-center justify-center text-[10px] opacity-40 italic uppercase">
                    {isLoading ? '...' : 'Signal Lost'}
                </div>
              )}
            </div>
            ...
            <div className="absolute bottom-6 left-0 right-0 px-6 flex justify-between items-center pointer-events-none">
              <button 
                ...
                className="pointer-events-auto h-7 px-4 bg-black text-white text-[8px] font-black uppercase active:invert transition-all shadow-md"
              >
                {mode === 'KEEP' ? 'FLIP' : 'NEXT'}
              </button>
              ...
              <div className="pointer-events-auto flex gap-1">
                <button 
                  onClick={() => {
                    setIsBiting(true);
                    setTimeout(() => setIsBiting(true), 200); // UI上の反応
                  ...
                  className="h-7 px-3 border border-black bg-white text-[8px] font-black italic active:bg-black active:text-white"
                >
                  BITE
                </button>
                <button 
        ...
        {mode === 'POST' && (
            <div className="h-full flex flex-col p-6 bg-white border border-black overflow-hidden">
                ...
                <textarea value={postInput.body} onChange={(e) => setPostInput(prev => ({...prev, body: e.target.value}))} placeholder="WORDS..." className="flex-grow outline-none text-sm font-bold resize-none mb-4" />
                <button onClick={handlePost} className="h-8 bg-black text-white text-[8px] font-black uppercase tracking-widest">DEPOSIT</button>
            </div>
        )}
      </main>
