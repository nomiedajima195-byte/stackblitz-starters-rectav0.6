...
export default function EngineMinimalButtons() {
  ...
  const getBiteMask = (count: number) => {
    ...
    return {
      maskImage: 'radial-gradient(circle 60px at calc(100% - 5px) 5px, transparent 100%, black 100%)',
      WebkitMaskImage: 'radial-gradient(circle 60px at calc(100% - 5px) 5px, transparent 100%, black 100%)',
    };
  };
  ...
  return (
    <div className="bg-white text-black font-mono h-[100dvh] flex flex-col overflow-hidden select-none border-x-2 border-black">
      <header className="h-8 border-b border-black flex items-center justify-between px-4 shrink-0 bg-white z-20">
        ...
        <span className="text-[8px] font-bold uppercase tracking-widest">{mode}</span>
      </header>

      <main className="flex-grow relative flex flex-col overflow-hidden bg-white">
        {(mode === 'MAIN' || mode === 'WIKI' || mode === 'KEEP') && (
          <div className={`flex flex-col h-full relative transition-all duration-300 ${isBiting ? 'scale-[0.98]' : 'scale-100'}`}>
            <div 
              ...
              {(mode === 'MAIN' ? streetPost : (mode === 'WIKI' ? wikiData : keeps[currentIndex % (keeps.length || 1)])) ? (
                <article className={isBiting ? 'animate-shake' : ''}>
                  {(mode === 'MAIN' ? streetPost : (mode === 'WIKI' ? null : keeps[currentIndex % (keeps.length || 1)]))?.image && (
                    <img src={(mode === 'MAIN' ? streetPost : keeps[currentIndex % (keeps.length || 1)]).image} className="w-full h-auto border border-black mb-6 grayscale" alt="fragment" />
                  )}
                  ...
                  <p className="text-sm font-bold whitespace-pre-wrap leading-relaxed pb-24">
                    {(mode === 'MAIN' ? streetPost?.body : (mode === 'WIKI' ? wikiData.content : keeps[currentIndex % (keeps.length || 1)]?.body))}
                  </p>
                </article>
              ) : (
                <div className="h-full flex items-center justify-center text-[10px] italic opacity-20 uppercase">No_Signal</div>
              )}
            </div>
            ...
            <div className="absolute bottom-6 left-0 right-0 px-6 flex justify-between items-center bg-transparent pointer-events-none">
              <button 
                ...
                className="pointer-events-auto h-8 px-4 bg-black text-white text-[9px] font-black uppercase tracking-tighter active:invert transition-all"
              >
                {mode === 'KEEP' ? 'FLIP' : 'NEXT'}
              </button>
              
              <div className="pointer-events-auto flex gap-1">
                <button onClick={handleBite} className="h-8 px-3 border border-black bg-white text-[9px] font-black italic active:bg-black active:text-white">BITE</button>
                <button onClick={() => handleKeep(mode === 'MAIN' ? streetPost : (mode === 'WIKI' ? wikiData : null))} className="h-8 px-3 border border-black bg-white text-[9px] font-black active:bg-black active:text-white uppercase">KEEP</button>
              </div>
            </div>
          </div>
        )}
        ...
        {mode === 'POST' && (
          <div className="h-full flex flex-col p-6 animate-in fade-in duration-300">
             <input value={postInput.title} onChange={(e) => setPostInput({...postInput, title: e.target.value})} placeholder="TITLE..." className="border-b border-black py-2 outline-none font-black text-xl uppercase mb-6" />
             ...
             <div className="flex gap-2">
               <button onClick={() => fileInputRef.current?.click()} className="h-8 px-4 border border-black text-[9px] font-black uppercase">Img</button>
               ...
               <button onClick={handlePost} className="flex-grow h-8 bg-black text-white text-[9px] font-black uppercase" disabled={isLoading}>Deposit</button>
             </div>
          </div>
        )}
      </main>
      ...
      <footer className="h-10 border-t border-black flex bg-white shrink-0">
        {['MAIN', 'POST', 'WIKI', 'KEEP'].map((m) => (
          <button key={m} onClick={() => setMode(m)} className={`flex-1 flex items-center justify-center border-r border-black last:border-0 ${mode === m ? 'bg-black text-white' : 'bg-white'}`}>
            <span className="text-[10px] font-black tracking-widest uppercase">{m === 'MAIN' ? 'Street' : m}</span>
          </button>
        ))}
      </footer>
