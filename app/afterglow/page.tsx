...
export default function EngineArticleFirst() {
    ...
    <div className="bg-white text-black font-mono h-[100dvh] flex flex-col overflow-hidden select-none border-x-2 border-black">
      ...
      <header className="h-10 border-b-2 border-black flex items-center justify-between px-4 shrink-0">
        ...
        <div className="flex items-center gap-2">
           ...
           <span className="text-[8px] font-black animate-pulse">● LIVE</span>
        </div>
      </header>

      <main className="flex-grow relative flex flex-col overflow-hidden">
        {/* Contents Area (MAIN, WIKI, KEEP) */}
        {(mode === 'MAIN' || mode === 'WIKI' || mode === 'KEEP') && (
          <div className="flex flex-col h-full">
            <div className="flex-grow overflow-y-auto custom-scrollbar p-4 pb-20">
              {/* TTL Badge Floating */}
              ...
              <div className="animate-in fade-in duration-500">
                {mode === 'MAIN' && streetPost ? (
                  ...
                  <article>
                    <h2 className="text-3xl font-black mb-6 underline decoration-4 leading-[1.1]">{wikiData.title}</h2>
                ...
                ) : <div className="h-64 flex items-center justify-center italic text-gray-300">NO SIGNALS...</div>}
              </div>
            </div>

            {/* Float Action Overlay (記事を邪魔しない) */}
            <div className="absolute bottom-4 left-4 right-4 flex gap-2">
               ...
               {mode === 'KEEP' && <button onClick={() => setCurrentIndex(prev => prev + 1)} className="flex-[3] h-14 bg-black text-white font-black text-lg active:scale-95 transition-transform uppercase tracking-tighter">Flip</button>}
        ...
        {/* POST MODE (Simplified) */}
        {mode === 'POST' && (
          <div className="h-full flex flex-col p-4 animate-in slide-in-from-bottom-4">
             ...
             <div className="flex-grow flex flex-col overflow-hidden mb-4">
               ...
               <textarea value={postInput.body} onChange={(e) => setPostInput({...postInput, body: e.target.value})} placeholder="捨てる言葉を..." className="flex-grow outline-none text-lg font-bold resize-none p-2 border-2 border-black" />
             </div>
