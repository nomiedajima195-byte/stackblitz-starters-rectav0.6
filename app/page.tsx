        ...
        <div className="grid grid-cols-4 gap-3 mb-10">
          {pads.map((p:any, i:number) => (
            <div key={i} onMouseDown={()=>triggerPad(i)} className={`aspect-square rounded-2xl border transition-all flex items-center justify-center relative overflow-hidden active:scale-90 cursor-pointer ${p ? 'bg-white border-black/5 shadow-sm' : 'bg-black/5 border-dashed border-black/10'}`}>
              {p?.image_url ? <img src={p.image_url} className="w-full h-full object-cover opacity-60" /> : <label className="w-full h-full flex items-center justify-center cursor-pointer opacity-10 hover:opacity-40 transition-opacity">＋<input type="file" className="hidden" onChange={e=>e.target.files?.[0] && handleFile(i, e.target.files[0])} /></label>}
            </div>
          ))}
        </div>
        <button onClick={()=>onRelease({description: JSON.stringify(trackData)})} className="w-full py-4 bg-black text-white text-[9px] font-black uppercase tracking-[0.3em] rounded-2xl mb-2">Release Track</button>
  ...
  return (
    <div className="fixed inset-0 z-[4500] bg-[#EBE8DB] p-4 flex flex-col items-center justify-center animate-in fade-in">
       ...
       <div className="mt-12 flex items-center space-x-10">
          ...
          <button onClick={async ()=>{setLoading(true); await onRelease({description: JSON.stringify(boxItems.filter(i=>i!==null))});}} disabled={loading} className="px-10 py-4 bg-black text-white text-[10px] font-black uppercase rounded-full tracking-[0.2em] shadow-2xl active:scale-95 transition-all">{loading ? '...' : 'Release Box'}</button>
       </div>
    </div>
  );
}
