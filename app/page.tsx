...
export default function Page() {
  ...
  const uploadFile = async (e: React.ChangeEvent<HTMLInputElement>, parentId: string | null = null) => {
    ...
    img.onload = async () => {
      ...
      const targetW = 400;
      const targetH = Math.round(400 * 1.618);
    ...
    <div className={`min-h-screen bg-[#F2F2F2] text-black font-sans ${isDeepInAlley ? 'overflow-hidden' : 'overflow-x-hidden'}`}>
      ...
      {!isMineMode ? (
        <div className="max-w-md mx-auto relative min-h-screen">
          <header className={`fixed top-0 left-0 right-0 h-16 z-50 flex justify-center items-center transition-all duration-500`}>
            <div onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} className={`w-[10px] h-[20px] bg-black cursor-pointer transition-opacity duration-500 ${isAtTop && !isDeepInAlley ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} />
            <div onClick={backToMainline} className={`absolute left-8 text-[22px] font-light cursor-pointer transition-opacity duration-500 ${isDeepInAlley ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>＜</div>
          </header>
          ...
          <nav className={`fixed bottom-0 left-0 right-0 h-20 bg-gradient-to-t from-[#F2F2F2] to-transparent z-50 flex justify-center items-center transition-opacity duration-500 ${isDeepInAlley ? 'opacity-0' : 'opacity-100'}`}>
            <label className={`w-10 h-10 border-[1px] border-black rounded-full flex items-center justify-center cursor-pointer bg-white/50 backdrop-blur-sm ${isUploading ? 'animate-pulse' : ''}`}>
